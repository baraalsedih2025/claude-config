...
HOST_FILE="$REPO_DIR/hosts/$(hostname).md"
if [ ! -e "$HOST_FILE" ]; then
  log "note: no hosts/$(hostname).md in the repo (fine — create one if this host needs overrides)"
else
  log "per-host file present: hosts/$(hostname).md (not auto-linked; include it from CLAUDE.md if wanted)"
fi
